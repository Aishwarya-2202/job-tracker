    ...
    <nav className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm">
      <div className="flex items-center gap-8">
        ...
        <div className="flex gap-6">
          ...
          <Link to="/add-job" className="text-gray-600 hover:text-indigo-600 font-medium transition-colors">
            Add Job
          </Link>
          {user?.role === 'admin' && (
      ...
      <div className="flex items-center gap-4">
        <span className="text-sm text-gray-500">
          ...
          {user?.role === 'admin' && (
            <span className="ml-2 bg-indigo-100 text-indigo-700 text-xs px-2 py-0.5 rounded-full font-medium">Admin</span>
          )}
        </span>
        <button
